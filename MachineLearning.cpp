	...
	bool transpose_a, bool transpose_b)
{
	...
	{
		mat_mul_cuda(out, a, b);
		return true;
	}
#endif

	// fallback to CPU for other transpose cases
...
bool mat_softmax(matrix* out, const matrix* in) {
	if (!out || !in || out->rows != in->rows || out->cols != in->cols) {
		return false;
	}

#ifdef USE_CUDA
	// call the CUDA softmax implementation
	mat_softmax_cuda(out, in);
	return true;
#else
	// CPU fallback
	u64 size = (u64)out->rows * out->cols;
...
#ifdef USE_CUDA
	mat_cross_entropy_cuda(out, p, q);
	return true;
#else
	u64 size = (u64)out->rows * out->cols;
	for (u64 i = 0; i < size; i++) {
...
model_var* mv_create(mem_arena* arena, model_context* model, u32 rows, u32 cols, u32 flags) {
	...
	if (flags & MV_FLAG_PARAMETER) {
		out->m = mat_create(arena, rows, cols);
		out->v = mat_create(arena, rows, cols);
		// initialize to zero
		mat_clear(out->m);
		mat_clear(out->v);
	}
	...
	for (u32 epoch = 0; epoch < training_desc->epochs; epoch++) {
		...
		for (u32 batch = 0; batch < num_batches; batch++) {
			...
			*/
			...
			//Adam parameters
			const f32 beta1 = 0.9f;
			const f32 beta2 = 0.999f;
			const f32 epsilon = 1e-8f;
			const f32 lr = training_desc->learning_rate;
			
			static u32 adam_timestep = 0;
			adam_timestep += 1;

			f32 bias_correction1 = 1.0f - powf(beta1, (f32)adam_timestep);
			f32 bias_correction2 = 1.0f - powf(beta2, (f32)adam_timestep);
			...
			for (u32 i = 0; i < model->cost_prog.size; i++) {
				...
				if (!cur->m || !cur->v || !cur->grad || !cur->val) {
					continue;
				}
				u64 size = (u64)cur->val->rows * cur->val->cols;

				f32 inv_batch = 1.0f / (f32)training_desc->batch_size;

				for (u64 idx = 0; idx < size; idx++) {
					//average gradient for parameter element
					f32 g = cur->grad->data[idx] * inv_batch;
					//calc m
					cur->m->data[idx] = beta1 * cur->m->data[idx] + (1.0f - beta1) * g;
					//calc v
					cur->v->data[idx] = beta2 * cur->v->data[idx] + (1.0f - beta2) * g * g;

					f32 m_hat = cur->m->data[idx] / bias_correction1;
					f32 v_hat = cur->v->data[idx] / bias_correction2;

					cur->val->data[idx] -= lr * (m_hat / (sqrtf(v_hat) + epsilon));
				}
			}
			printf(
